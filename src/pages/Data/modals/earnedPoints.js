...
const steps = [
  {
    ...
    Component: (props) => {
      ...
      return (
        <Fragment>
          <Field label={<FormattedMessage id="enter_points" />}>
            <Input
              inputProps={{ min: 0 }}
              fullWidth
              type="number"
...
export const EarnedPointsBody = (props) => {
  ...
  const handleSubmit = async () => {
    ...
    state.selectedKids.map(async (kid) => {
      ...
      const currentXp = state.score;
      const newLevel = Math.trunc(currentXp / xpToGainNewLevel);
      const xpNextNew = (newLevel + 1) * xpToGainNewLevel;
      const xpPrevNew =
        newLevel == 0
          ? 0
          : newLevel == 1
          ? xpToGainNewLevel
          : newLevel * xpToGainNewLevel;

      batch.update(ref, {
        xp: Number(state.score),
        level: newLevel == 0 ? 1 : newLevel + 1,
        xpForPreviousLevel: xpPrevNew,
        xpForNextLevel: xpNextNew,
      });
    });
